...
int DCA(uint8_t map[256])
{
    ...
    for(int j = 0; j < 256; j++)//key
    {
        ...
        {
            double split0[32] = {0.0};
            double split1[32] = {0.0};
            int count0 = 0;
            int count1 = 0;
...
int BCA(uint8_t map[256])
{
    ...
    uint8_t sortinput[256][32];
    for(int k = 0; k < 256; k++)
    {
        int I0 = 0, I1 = 16;
        for(int x = 0; x < 256; x++)
        {
            if((SBox[x ^ k] & 0x0f) == 0) sortinput[k][I0++] = x;
            if((SBox[x ^ k] & 0x0f) == 0x0f) sortinput[k][I1++] = x;
        }
    }
    ...
    for(int k = 0; k < 256; k++)
    {
        for(int s = 0; s < 8; s++)
        {
            ...
            for(int i = 0; i < 16; i++)
            {
                if(trace[sortinput[k][i]][s] == trace[sortinput[k][16 + i]][s])
                {
                    flag = 0;
                    break;
                }
    ...
    int key_guess_value = 0;
    int key_guess = 0;
    for(int k = 0; k < 256; k++)
    {
        if(key_count[k] > key_guess_value) // maximum
        {
            key_guess_value = key_count[k];
            key_guess = k;
        }
    }
    if(key_guess == real_key) return 1;
